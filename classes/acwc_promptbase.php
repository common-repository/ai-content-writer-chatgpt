...
if(!class_exists('\\ACWC\\ACWC_Promptbase')) {
    class ACWC_Promptbase
    {
        ...
        public function create_table_logs()
        {
            global $wpdb;
            if(is_admin()) {
                $acwcLogTable = $wpdb->prefix . 'acwc_promptbase_logs';
    ...
    PRIMARY KEY  (id)
        ...
        public function __construct()
        {
            ...
            add_shortcode('acwc_prompt',[$this,'acwc_prompt_shortcode']);
            add_action('wp_ajax_acwc_update_prompt',[$this,'acwc_update_prompt']);
            add_action('wp_ajax_acwc_prompt_delete',[$this,'acwc_prompt_delete']);
            add_action('wp_ajax_acwc_prompt_log', [$this,'acwc_prompt_log']);
            add_action('wp_ajax_nopriv_acwc_prompt_log', [$this,'acwc_prompt_log']);
            if ( ! wp_next_scheduled( 'acwc_remove_promptbase_tokens_limited' ) ) {
                wp_schedule_event( time(), 'hourly', 'acwc_remove_promptbase_tokens_limited' );
            }

            $this->create_table_logs();
        }

        public function acwc_prompt_log()
        {
            global $wpdb;
            $acwc_result = array('status' => 'success');
            $acwc_nonce = sanitize_text_field($_REQUEST['_wpnonce']);
            if ( !wp_verify_nonce( $acwc_nonce, 'acwc-promptbase' ) ) {
            ...
            if(
                ...
                $wpdb->insert($wpdb->prefix.'acwc_promptbase_logs', $log);
        ...
        public function acwc_menu()
        {
            add_submenu_page(
                'acwc',
                'OpenAI Prompt Contant Generator',
                'Prompt Content Generator',
                'manage_options',
                'acwc_promptbase',
                array( $this, 'acwc_promptbase'),
                6
            );
        }

        public function acwc_update_prompt()
        {
            $acwc_result = array('status' => 'error', 'msg' => 'Something went wrong');
            if ( ! wp_verify_nonce( $_POST['_wpnonce'], 'acwc_promptbase_save' ) ) {
            ...
            if(
                ...
                foreach($prompt_fields as $prompt_field){
                    if(isset($_POST[$prompt_field]) && !empty($_POST[$prompt_field])){
                        $value = acwc_util_core()->sanitize_text_or_array_field($_POST[$prompt_field]);
                        $key = sanitize_text_field($prompt_field);
                        update_post_meta($acwc_prompt_id, 'acwc_prompt_'.$key, $value);
                    }
                    elseif(in_array($prompt_field,array('bgcolor','header','dans','ddraft','dclear','dnotice')) && (!isset($_POST[$prompt_field]) || empty($_POST[$prompt_field]))){
                        delete_post_meta($acwc_prompt_id, 'acwc_prompt_'.$prompt_field);
                    }
                }
        ...
        public function acwc_prompt_delete()
        {
            $acwc_result = array('status' => 'success');
        ...
        public function acwc_promptbase()
        {
            include ACWC_PLUGIN_DIR . 'backend/acwc_promptbase.php';
        }

        public function acwc_prompt_shortcode($atts)
        {
            ob_start();
            include ACWC_PLUGIN_DIR . 'backend/acwc_prompt_shortcode.php';
            return ob_get_clean();
        }
    }
    ACWC_Promptbase::get_instance();
}
