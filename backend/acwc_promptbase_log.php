...
global $wpdb;
if (isset($_GET['acwc_nonce']) && !wp_verify_nonce($_GET['acwc_nonce'], 'acwc_promptbaselog_search_nonce')) {
...
$query = "SELECT * FROM ".$wpdb->prefix."acwc_promptbase_logs WHERE 1=1".$where;
...
<form action="" method="get" style="margin-top:50px;">
    <?php wp_nonce_field('acwc_promptbaselog_search_nonce', 'acwc_nonce'); ?>
    <input type="hidden" name="page" value="acwc_promptbase">
    <input type="hidden" name="action" value="logs">
    <div class="d_flex mb_5">
        <input style="width: 100%;padding:5px;" value="<?php echo esc_html($search)?>" class="" name="search" type="text" placeholder="Type for search">
...
<div class="acwc_paginate">
    ...
    if($totalPage > 1){
        echo paginate_links( array(
            'base'         => admin_url('admin.php?page=acwc_promptbase&action=logs&wpage=%#%'),
            'total'        => $totalPage,
